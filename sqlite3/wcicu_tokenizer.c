...
int fts5WcicuTokenize(
	...
	// strcpy(dText, pText);
	// trim(dText);
	// nText = (int)strlen(dText);
	// after:
	char *dText = (char *)malloc(sizeof(char) * (nText + 1));
	strcpy(dText, pText);
	trim(dText);
	nText = (int)strlen(dText);
	// ==========================================
    ...
    //break by 'word'
    //raymond cur->iter = ubrk_open(UBRK_CHARACTER, tok->locale, cur->in_buffer, i_output, &status);
...
static int output_token(mm_cursor_t *cur,
    ...
    *piPosition = ++cur->token_count;
	//raymond *piPosition = cur->token_count++;
